...
class CustomDataset(IterableDataset):
    def __init__(self, args, dataset_type='train'):
        ...
        if dataset_type == 'train':
            source_split = split_source_into_three(Source_data[:, :train_steps_source, :])
            target_data = Target_data[:, :train_steps_target, :]
        elif dataset_type == 'val':
            source_split = split_source_into_three(Source_data[:, train_steps_source : train_steps_source+val_steps_source, :])
            target_data = Target_data[:, train_steps_target : train_steps_target+val_steps_target, :]
        elif dataset_type == 'test':
            source_split = split_source_into_three(Source_data[:, train_steps_source+val_steps_source : train_steps_source+val_steps_source+test_steps_source, :])
