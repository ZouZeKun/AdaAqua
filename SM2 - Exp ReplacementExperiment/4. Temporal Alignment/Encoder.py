...
class TemporalAlignment(nn.Module):
    def __init__(self, args):
        ...
        self.fc_3 = nn.Sequential(
            nn.Linear(self.seq_lens, self.hidden_dim),
            nn.Tanh(),
            nn.Linear(self.hidden_dim, self.seq_lens),
            )
        
    def forward(self, x):

        # 外部建筑环境特征与用水模式特征同时偏移
        x_1 = (self.fc_1(x.transpose(1,2))).transpose(1,2)
        x_2 = (self.fc_2(x.transpose(1,2))).transpose(1,2)
        x_3 = (self.fc_3(x))

        x = torch.cat((x, x_1, x_2, x_3), dim=1)  # (199,96,4*hidden_dims)
