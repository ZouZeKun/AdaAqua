...
def test(args, IO, test_loader, output_dir):
    ...
    numbers = args.num_buildings_source
    ...
    for batch_idx, data in enumerate(tqdm(test_loader, total=int(96*8*3 - 95*3), desc="Test_Loader")):
        ...
        y_test = y2[fold_from:fold_to, :, :].squeeze(-1) + Mean_Tensor[(numbers+fold_from):(numbers+fold_to), :] 
        ...
        pred_all = pred_all[(numbers+fold_from):(numbers+fold_to), :] + Mean_Tensor[(numbers+fold_from):(numbers+fold_to), :]
