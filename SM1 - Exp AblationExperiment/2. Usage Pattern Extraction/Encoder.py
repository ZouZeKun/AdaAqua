...
class AttentionTransfer(nn.Module):
    def __init__(self, args):
        ...
        self.fc_q1 = nn.Linear(self.hidden_dims+1, self.hidden_dims)
        self.fc_q2 = nn.Linear(self.hidden_dims+1, self.hidden_dims)
        self.fc_q3 = nn.Linear(self.hidden_dims+1, self.hidden_dims)
        self.fc_q4 = nn.Linear(self.hidden_dims+1, self.hidden_dims)
        
        self.fc_k1 = nn.Linear(self.hidden_dims+1, self.hidden_dims)
        self.fc_k2 = nn.Linear(self.hidden_dims+1, self.hidden_dims)
        self.fc_k3 = nn.Linear(self.hidden_dims+1, self.hidden_dims)
        self.fc_k4 = nn.Linear(self.hidden_dims+1, self.hidden_dims)
        
        self.fc_v1 = nn.Linear(self.hidden_dims+1, self.hidden_dims//4)
        self.fc_v2 = nn.Linear(self.hidden_dims+1, self.hidden_dims//4)
        self.fc_v3 = nn.Linear(self.hidden_dims+1, self.hidden_dims//4)
        self.fc_v4 = nn.Linear(self.hidden_dims+1, self.hidden_dims//4)
...
class FeatMigrator(nn.Module):
    ...
    def forward(self, Features, Patterns, mask):
        ...
        # 基于注意力机制的特征迁移
        out = Features  # (199,96,hidden_dims+1)
