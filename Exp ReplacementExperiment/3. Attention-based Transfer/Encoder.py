...
class FourierProjection(nn.Module):
    ...
    def forward(self, x):
        ...
        s = self.sin_out(s)  # 最终升维
        for block in self.cos_blocks:
            c = block(c) + residual_c
        c = self.cos_out(c)  # 最终升维

        y = torch.cat((s, c), dim=-1)  # (B, D, 2*hidden_dims)
...
class AttentionTransfer(nn.Module):
    def __init__(self, args):
        ...
        self.fc_q1 = nn.Linear(2*self.hidden_dims+1, 2*self.hidden_dims+1)
        self.fc_q2 = nn.Linear(2*self.hidden_dims+1, 2*self.hidden_dims+1)
        self.fc_q3 = nn.Linear(2*self.hidden_dims+1, 2*self.hidden_dims+1)
        self.fc_q4 = nn.Linear(2*self.hidden_dims+1, 2*self.hidden_dims+1)
        
        self.fc_k1 = nn.Linear(2*self.hidden_dims+1, self.hidden_dims//4)
        self.fc_k2 = nn.Linear(2*self.hidden_dims+1, self.hidden_dims//4)
        self.fc_k3 = nn.Linear(2*self.hidden_dims+1, self.hidden_dims//4)
        self.fc_k4 = nn.Linear(2*self.hidden_dims+1, self.hidden_dims//4) 
    ...
    def forward(self, x):
        ...
        x_k1 = self.fc_k1(x_q1)
        x_k2 = self.fc_k2(x_q2)
        x_k3 = self.fc_k3(x_q3)
        x_k4 = self.fc_k4(x_q4)

        x1 = x_k1.permute(1, 2, 0)
        x2 = x_k2.permute(1, 2, 0)
        x3 = x_k3.permute(1, 2, 0)
        x4 = x_k4.permute(1, 2, 0)
...
class TemporalAlignment(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.cat((x, x_3, x_5, x_7), dim=1)  # (199,96,4*hidden_dims)

        return x.transpose(1,2)  # (199,96,4)
...
class FeatMigrator(nn.Module):
    ...
    def forward(self, Features, Patterns, mask):
        ...
        Features = torch.cat([Features, mask], dim=-1)  # (199,96,16)

        # 特征自注意力层(迁移)
        Features = self.Feature(Features)      # (199,96,12)
        Features = self.Position_Embedding(Features)  # (199,96,hidden_dims+1)
        # print("FeatMigrator —— Features:", Features.shape)
        
        # FourierProjection
        Time = self.FP(Patterns.squeeze(-1))  # (199,96,hidden_dims)
        # print("FeatMigrator —— Time:", Time.shape)

        # 基于注意力机制的特征迁移
        out = torch.cat([Features, Time], dim=-1)  # (199,96,2*hidden_dims+1)
        # print("FeatMigrator —— out:", out.shape)

        # 特征融合
        out = self.Transfer(out) # (199,96,hidden_dims+1+2+2)
        # print("FeatMigrator —— out:", out.shape)
