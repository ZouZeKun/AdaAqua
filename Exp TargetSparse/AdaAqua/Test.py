...
def test(args, IO, test_loader, output_dir):
    ...
    for batch_idx, data in enumerate(tqdm(test_loader, total=int(96*8*3 - 95*3), desc="Test_Loader")):
        ...
        if args.fold != 2:
            ...
            fold_to = args.fold * 8 + 16
            y_test = y2[fold_from:fold_to, :, :].squeeze(-1) + Mean_Tensor[(199+fold_from):(199+fold_to), :] 
            y2[fold_from:fold_to, :, :] = 0
        else:
            y_test1 = y2[0:8, :, :].squeeze(-1)   + Mean_Tensor[199:199+8, :]
            y_test2 = y2[16:24, :, :].squeeze(-1) + Mean_Tensor[199+16:199+24, :]
        ...
        if args.fold != 2:
            fold_from = args.fold * 8
            fold_to = args.fold * 8 + 16
            mask_source_[fold_from:fold_to, :, :] = 1
        else:
            mask_source_[0:8, :, :] = 1
            mask_source_[16:24, :, :] = 1
        ...
        if args.fold != 2:
            ...
            pred_all1 = pred_all[199+0:199+8, :]   + Mean_Tensor[199+0:199+8, :]
            pred_all2 = pred_all[199+16:199+24, :] + Mean_Tensor[199+16:199+24, :]
